...
class ExecutionEngine:
    ...
    def __init__(
        ...
        self.max_pnl_pct: float = 0.0  # 최대 수익률 추적 (trailing stop용)
    ...
    def _check_stop_loss_take_profit(self, current_price: float) -> StrategySignal | None:
        """
        Stop-Loss와 Take-Profit을 체크하여 강제 청산 시그널 반환.

        Args:
            current_price: 현재 가격

        Returns:
            StrategySignal.SELL if stop-loss or take-profit triggered, else None
        """
        if self.position_price is None or self.risk_manager is None:
            return None
        ...
        if pnl_pct <= risk_config.stop_loss_pct:
            LOGGER.warning(
                "Stop-Loss triggered for %s: %.2f%% (threshold: %.2f%%)",
                self.market,
                pnl_pct,
                risk_config.stop_loss_pct,
            )
            self._notify(
                f"⚠️ STOP-LOSS: {self.market}",
                pnl_pct=pnl_pct,
                price=current_price,
                reason="stop_loss",
            )
        ...
        if pnl_pct >= risk_config.take_profit_pct:
            LOGGER.info(
                "Take-Profit triggered for %s: %.2f%% (threshold: %.2f%%)",
                self.market,
                pnl_pct,
                risk_config.take_profit_pct,
            )
            self._notify(
                f"✅ TAKE-PROFIT: {self.market}",
                pnl_pct=pnl_pct,
                price=current_price,
                reason="take_profit",
            )
        ...
        if risk_config.trailing_stop_pct is not None and self.max_pnl_pct > 0:
            trailing_trigger = self.max_pnl_pct - risk_config.trailing_stop_pct
            if pnl_pct <= trailing_trigger:
                LOGGER.info(
                    "Trailing Stop triggered for %s: %.2f%% (max: %.2f%%, trigger: %.2f%%)",
                    self.market,
                    pnl_pct,
                    self.max_pnl_pct,
                    trailing_trigger,
    ...
    def run_once(self) -> dict | None:
        ...
        # 포지션이 열려있으면 Stop-Loss/Take-Profit 먼저 체크
        sl_tp_signal = self._check_stop_loss_take_profit(current_price)
        if sl_tp_signal is StrategySignal.SELL:
            signal = sl_tp_signal
            LOGGER.info("Risk management override: %s", signal.value)
        else:
        ...
        try:
            ...
            # 포지션 청산 시 max_pnl_pct 초기화
            if signal is StrategySignal.SELL and self.position_price is None:
